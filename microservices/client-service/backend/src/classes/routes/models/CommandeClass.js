...
class Commande {
    ...
    static async find() {
        ...
        const t = await CommandeSchema.find().populate('client').sort({ $natural: -1 }).exec()
        return t
    }
    ...
    static async findByLivreur(livreurId) {

        const t = await CommandeSchema.find({livreur: livreurId}).sort({ $natural: -1 }).exec()
        return t
    }
