...
import { getDeveloperbyemail } from '../api/developer';
...
export default function DashboardAppPage() {
  ...
  useEffect(() => {
    if (userInfo && userInfo.email) {
      getDeveloperbyemail(userInfo.email)
        .then((res) => {
          setDeveloper(res.data);
          console.log(res.data)
          localStorage.setItem('developer', JSON.stringify(res.data)); // Store restaurant data in local storage
          setLoading(false);
        })
        .catch((error) => {
      ...
      <div>
        <div>
          <Typography variant="h3" sx={{ mb: 5 }}>
            Welcome, {developer && developer.name}!
          </Typography>
