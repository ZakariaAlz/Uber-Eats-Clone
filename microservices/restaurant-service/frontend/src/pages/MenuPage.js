...
import { getMenubyRestaurant, deleteMenu } from '../api/menu'
...
export default function UserPage() {
  ...
  useEffect(() => {
    getMenubyRestaurant(restaurant._id)
      .then((res) => {
        setMenus(res.data)
        setLoading(false)
      })
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = menus.map((n) => n.firstname);
  ...
  const filteredAdmins = applySortFilter(menus, getComparator(order, orderBy), filterName);
                ...
                <TableBody>
                  {loading ? (
                    ...
                    filteredAdmins
                      ...
                      .map((menu) => {
                        const { _id, name, description, price } = menu;
                        ...
                        return (
                          <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={isItemSelected}>
                            ...
                            <TableCell align="right">
                              ...
                              <Link to="/dashboard/update-menu" state={{
                                id: _id,
                                Name: name,
                              ...
                              <IconButton style={{ color: "red" }} onClick={() => {
                                if (window.confirm('Sure you want to delete this Menu ?')) {
